...
- Issue 작성 시 해당 작업에 적용할 skill을 명시한다.
...
- 다른 agent가 이어받을 수 있게 다음 작업 위치와 남은 리스크를 남긴다.

### 모호함 처리 규칙

- `What`(무엇을 만드는가)이 모호하면 개발을 시작하지 않는다. 사람에게 명확화를 요청한다.
- `How`(어떻게 만드는가)가 모호하면 가정을 명시하고 진행한다. 가정은 커밋 메시지 또는 PR에 기록한다.

### 작업 완료 후 전달

작업 완료 후 AI는 다음을 요약해서 전달한다:
- 완료한 것
- 진행 중 내린 주요 가정
- 사용자 확인이 필요한 항목

전달 채널은 프로젝트 환경에 맞게 채운다. (예: PR, 대화창, 메시지 등)
